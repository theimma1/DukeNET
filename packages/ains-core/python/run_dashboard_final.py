...
async def update_metrics_loop():
    """Generate metrics data continuously"""
    global iteration
...
async def lifespan(app: FastAPI):
    ...
    yield
    # Shutdown
    print("✅ Stopping metrics update loop...")
    task.cancel()
    try:
        await task
  ...
  <script>
    ...
    function updateDashboard(data) {
      ...
      if (data.agents_detail) {
        ...
        if (count === 0) {
          document.getElementById('agentsList').innerHTML = '<p style="color: #cbd5e1; padding: 1rem;">No agents connected yet</p>';
        } else {
